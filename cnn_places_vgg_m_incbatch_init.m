function net = cnn_places_vgg_m_init(varargin)
...
switch opts.model
  ...
  case 'vgg-m'
%     net.normalization.imageSize = [224, 224, 3] ;
    net.normalization.imageSize = [96, 96, 3] ;
    net = vgg_m(net, opts) ;
  case 'vgg-m-dropout'
    net.normalization.imageSize = [96, 96, 3] ;
    net = vgg_m_dropout(net, opts) ;
  case 'vgg-m-dropv2'
    net.normalization.imageSize = [96, 96, 3] ;
    net = vgg_m_dropout(net, opts) ;
  case 'vgg-m-dropv3'
    net.normalization.imageSize = [96, 96, 3] ;
    net = vgg_m_dropv3(net, opts) ;
  case 'vgg-s'
    net.normalization.imageSize = [224, 224, 3] ;
    net = vgg_s(net, opts) ;
  case 'vgg-vd-16'
...
%net = add_block(net, opts, '6', 6, 6, 512, 4096, 1, 0) ;
% RESIZED FOR NEW IMAGE SIZE
net = add_block(net, opts, '6', 2, 2, 512, 4096, 1, 0) ;
net = add_dropout(net, opts, '6') ;
...
% --------------------------------------------------------------------
function net = vgg_m_dropout(net, opts)
% --------------------------------------------------------------------

net.layers = {} ;
net = add_block(net, opts, '1', 7, 7, 3, 96, 2, 0) ;
net = add_dropout(net, opts, 'dropout_1') ;
net = add_norm(net, opts, '1') ;
net.layers{end+1} = struct('type', 'pool', 'name', 'pool1', ...
                           'method', 'max', ...
                           'pool', [3 3], ...
...
net = add_block(net, opts, '2', 5, 5, 96, 256, 2, 1) ;
net = add_dropout(net, opts, 'dropout_2') ;
net = add_norm(net, opts, '2') ;
net.layers{end+1} = struct('type', 'pool', 'name', 'pool2', ...
                           'method', 'max', ...
                           'pool', [3 3], ...
...
%net = add_block(net, opts, '3', 3, 3, 256, 512, 1, 1) ;
%net = add_dropout(net, opts, 'dropout_3') ;

%net = add_block(net, opts, '4', 3, 3, 512, 512, 1, 1) ;
%net = add_dropout(net, opts, 'dropout_4') ;

net = add_block(net, opts, '5', 3, 3, 256, 512, 1, 1) ;
net = add_dropout(net, opts, 'dropout_5') ;
net.layers{end+1} = struct('type', 'pool', 'name', 'pool5', ...
                           'method', 'max', ...
                           'pool', [3 3], ...
                           'stride', 2, ...
...
net = add_block(net, opts, '5', 2, 2, 512, 4096, 1, 0) ;
net = add_dropout(net, opts, 'dropout_6') ;

net = add_block(net, opts, '7', 1, 1, 4096, 4096, 1, 0) ;
net = add_dropout(net, opts, 'dropout_7') ;

net = add_block(net, opts, '6', 1, 1, 4096, 1000, 1, 0) ;
net.layers(end) = [] ;
if opts.batchNormalization, net.layers(end) = [] ; end

% --------------------------------------------------------------------
function net = vgg_m_dropv3(net, opts)
% --------------------------------------------------------------------
...
net = add_block(net, opts, '5', 1, 1, 4096, 1000, 1, 0) ;
net.layers(end) = [] ;
if opts.batchNormalization, net.layers(end) = [] ; end

% --------------------------------------------------------------------
function net = vgg_m_dropv2(net, opts)
% --------------------------------------------------------------------
